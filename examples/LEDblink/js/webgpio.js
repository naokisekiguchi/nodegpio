  ...
  write: function (value) {
    ...
    var writeGPIO = ()=> {
      return new Promise((resolve, reject)=>{
        this.value = value;
        window.WorkerOvserve.observe(`gpio.setValue.${this.portNumber}`, (workerData) => {
          resolve(workerData.value);
        });
  ...
  navigator.requestGPIOAccess = function () {
    ...
    //if (!navigator.mozGpio) {
      return new Promise(function(resolve,reject){
        ...
        window.WorkerOvserve.notify('gpio', {
          method: 'gpio.requestGPIOAccess'
        });
        window.WorkerOvserve.observe(`gpio.requestGPIOAccess`, (data)=>{
          console.log(data.value);
          if(data.value){
            var gpioAccess = new GPIOAccess();
            resolve(gpioAccess.GPIOAccessThen.then(()=> gpioAccess));
          }else{
            reject({"message":"websocket connection error."});
          }
        });
      });
      /*
...
if (window.Worker && window.WorkerOvserve) {
  ...
  var _worker = new Worker(`${current.substr(0, current.lastIndexOf('/'))}/nodeworker.gpio.js`);
